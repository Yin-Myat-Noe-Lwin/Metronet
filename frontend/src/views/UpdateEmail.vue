  ...
  <div class="verify-page">
    ...
    <div v-if="loading" class="verify-container">
      <div class="verify-card loading-card">
        ...
        <h3>{{ isEmailUpdate ? 'Updating Your Email' : 'Verifying Your Email' }}</h3>
        <p class="text-muted">{{ isEmailUpdate ? 'Just a moment while we update your email address...' : 'Just a moment while we confirm your email address...' }}</p>
    ...
    <div v-else-if="success" class="verify-container">
      <div class="verify-card success-card">
        ...
        <div class="badge">{{ isEmailUpdate ? 'Email Updated' : 'Verified' }}</div>
        <h2 class="verify-title">{{ isEmailUpdate ? 'Email Updated Successfully!' : 'Email Verified Successfully!' }}</h2>
        <p class="verify-message">
          {{ isEmailUpdate
            ? 'Your email address has been successfully updated. You can now use your new email to log in.'
            : 'Your email has been confirmed. You\'re all set to access your account and explore our services.'
          }}
        </p>
        <div class="verify-actions">
          <router-link :to="isEmailUpdate ? '/profile' : '/login'" class="btn-primary">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"/>
              <polyline points="10 17 15 12 10 7"/>
              <line x1="15" x2="3" y1="12" y2="12"/>
            </svg>
            {{ isEmailUpdate ? 'Go to Profile' : 'Get Started' }}
          </router-link>
    ...
    <div v-else class="verify-container">
      <div class="verify-card error-card">
        ...
        <h2 class="verify-title">{{ isEmailUpdate ? 'Email Update Failed' : 'Verification Failed' }}</h2>
        <p class="verify-message">{{ message || (isEmailUpdate ? 'The email update link is invalid or has expired. Please request a new email update.' : 'The verification link is invalid or has expired. Please request a new verification email.') }}</p>
        <div class="verify-actions">
          <router-link to="/" class="btn-secondary">Return Home</router-link>
        </div>
      </div>
...
export default {
  ...
  async mounted() {
    ...
    const type = this.$route.query.type || 'verify'

    // Check if this is an email update verification
    this.isEmailUpdate = type === 'update_email'
    this.email = this.$route.query.email || ''
    this.userId = this.$route.query.user_id || null

    if (!token) {
      this.loading = false
      this.success = false
      this.message = this.isEmailUpdate
        ? 'Invalid email update link. Please check your email for the correct link.'
    ...
    try {
      ...
      this.message = error.response?.data?.message
        || (this.isEmailUpdate
          ? 'Email update link is invalid or expired.'
